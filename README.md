...
| 关键字 | 说明 | 语法demo | 详细说明 |
| --- | --- | --- | --- |
| ${} | 取值操作 | ```${name}``` 表示取对象的name属性 | [3.1.1 取值操作] |
| and | 并且条件 | ```${name} = '张三' and ${age} > 20``` 表示名称为张三并年龄大于20 | [3.1.2 并且条件] |
| or | 或者条件 | ```${name} = '张三' or ${name} = '李四'``` 表示名称为张三或者李四 | [3.1.3 或者条件] |
| () | 括号运算 | ```(${name} = '张三' or ${name} = '李四') and ${age} > 20``` 表示先判断名称可为张三或者李四，然后再判断年龄大于20岁 | [3.1.4 括号运算] |
| in | 同时指定多个或者条件 | ```${name} in ('张三','李四')``` 表示名称为张三或者李四 | [3.1.5 同时指定多个或者条件] |
| between | 范围条件 | ```${age} between 20 and 30``` 表示年龄在[20,30]范围内 | [3.1.6 范围条件] |
| is null | 等于空 | ```${name} is null``` 表示姓名为null 等价于 ```${name} = null``` | [3.1.7 等于空] |
| is not null | 不等于空 | ```${name} is not null``` 表示姓名不为null 等价于 ```${name} != null``` | [3.1.8 不等于空] |
| F{} | 执行函数 | ```F{StrLen(${name})} > 2``` 表示取出对象的name属性值并且执行StrLen函数，并对函数结果进行大于2判断 | [3.1.9 执行函数] |
...
- 框架预定义的函数如下（支持自定义函数，具体可查看 [4.1 自定义函数]）：
